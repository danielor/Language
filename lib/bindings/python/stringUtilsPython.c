...
static PyObject * py_stringutils_length(PyObject * self, PyObject *args){
	...
	if(PyObject_TypeCheck(stringArg, &PyString_Type)){
		...
		buffer = PyUnicode_AS_UNICODE(stringArg);
	}else{
	...
	if(encodingArg != NULL){
		if(PyObject_TypeCheck(encodingArg, &PyInt_Type)){
			int potentialEncoding = PyInt_AS_INT(encodingArg);
 ...
 */
PyMODINIT_FUNC initStringUtils(void){
	Py_InitModule3("stringutils", stringutils_methods, "A package of string utility function for Language");
}
