...
// A function that checks if a character the stride length of the character
int testGetCharacterStrideLength(){
	int r;
	const char * buffer = convertCodePointToUTF8Binary(201);
	const char * buffer2 = convertCodePointToUTF8Binary(100);
	const char * buffer3 = convertCodePointToUTF8Binary(2500);
	const char * buffer4 = convertCodePointToUTF8Binary(69000);
	struct TypeEncodingTest typeTests[6] = {
		{"Y", ASCII, 1},{"Y", ISO_8859_1, 1},{(const char *)buffer,UTF8_BINARY, 2},
		{(const char *)buffer2, UTF8_BINARY, 1},{(const char *)buffer3,UTF8_BINARY,3},
		{(const char *)buffer4, UTF8_BINARY, 4}
	};
	for(r=0; r <6; r++){
		struct TypeEncodingTest test = typeTests[r];;
		if(getCharacterStrideLength(test.buffer, test.encoding) != test.expectedResult){
			return 0;
		}
	}
	return -1;
...
// A function that checks if a character is a punctuation mark in
// different encodings and languages.
int testIsPunctuationMarkInAlphabet(){
	int r;
	struct LanguageTest languageTests[] = {
				{",.-",ASCII,SPANISH,0},{",-?",UTF8_BINARY,FRENCH,1},{"->",ISO_8859_1,ENGLISH,1},
				{"!)]",UTF8_BINARY,FRENCH,1},
		};
	for(r = 0; r< 4; r++){
		struct LanguageTest test = languageTests[r];
		if(isPunctuationMarkInAlphabet(test.buffer, test.encoding, test.language) != test.expectedResult){
			return 0;
		}
	}
	return -1;
...
// A function that checks if a character is a punction mark in a sequence
// int different encodings and language
int testIsPunctuationMarkInAlphabetSequence(){
	int r;
	struct LanguageTest languageTests[] = {
				{",.-",ASCII,SPANISH,0},{",-?",UTF8_BINARY,FRENCH,1},{"-?",ISO_8859_1,ENGLISH,1},
				{"!)]",UTF8_BINARY,FRENCH,1},
	...
	for(r = 0; r< 4; r++){
		...
		if(isPunctuationMarkInAlphabetSequence(test.buffer, test.encoding, test.language) != test.expectedResult){
			return 0;
		}
	}
	return -1;
